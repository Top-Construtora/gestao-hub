...
class ContractModel {
  ...
  async create(contractData, userId) {
    ...
    try {
      ...
      if (services && services.length > 0) {
        ...
        } else if (installment_count === 1 && expected_payment_date) {
  ...
  async update(id, contractData, userId) {
    try {
      ...
      if (expected_payment_date !== undefined) updateData.expected_payment_date = expected_payment_date;
  ...
  async softDelete(id, userId) {
    try {
      ...
      console.log('✅ Contrato marcado como inativo');
      return true;
    } catch (error) {
  ...
  async generateContractNumber() {
    ...
    for (let attempt = 0; attempt < 10; attempt++) {
      try {
        ...
        let nextNumber = 1;
        if (data && data.length > 0) {
          const lastNumberMatch = data[0].contract_number.match(/NAUE-\d{4}-(\d+)/);
