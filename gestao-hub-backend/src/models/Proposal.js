...
class ProposalModel {
  ...
  async create(proposalData, userId) {
    ...
    try {
      ...
      // Calcular data de expiração do link (30 dias por padrão)
      const link_expires_at = new Date();
      link_expires_at.setDate(link_expires_at.getDate() + 30);
      ...
      const { data: proposal, error: proposalError } = await supabase
        ...
        .insert([{
          ...
          unique_link,
          link_expires_at: link_expires_at.toISOString(),
  ...
  async findAll(filters = {}) {
    try {
      let query = supabase
        .from('proposals')
        .select(`
          id, proposal_number, client_id, type, status, total_value, end_date, unique_link, link_expires_at, signed_at, signature_data, converted_to_contract_id,
          ...
          created_by_user:users!proposals_created_by_fkey(name),
          updated_by_user:users!proposals_updated_by_fkey(name)
        `)
  ...
  async findById(id) {
    try {
      const { data: proposal, error: proposalError } = await supabase
        .from('proposals')
        .select(`
          id, proposal_number, client_id, type, status, total_value, end_date,
          unique_link, link_expires_at, signed_at, signature_data,
      ...
      const { data: services, error: servicesError } = await supabase
        ...
        .select(`
          ...
          service:services(id, name, duration_amount, duration_unit, category, description, subtitle, summary)
        `)
        .eq('proposal_id', id)
        .order('sort_order', { ascending: true });
  ...
  async duplicate(id, duplicateOptions = {}, userId) {
    try {
      ...
      const newProposalData = {
        ...
        end_date: duplicateOptions.end_date || originalProposal.end_date,
  ...
  async findByPublicToken(token) {
    try {
      const { data: proposal, error: proposalError } = await supabase
        ...
        .select(`
          id, proposal_number, client_id, type, status, total_value, end_date,
          unique_link, link_expires_at, signed_at,
          converted_to_contract_id, created_at, updated_at,
          signer_name, signer_email, signer_phone, signer_document, signer_observations,
          payment_type, payment_method, installments, final_value, discount_applied, max_installments,
          vista_discount_percentage, prazo_discount_percentage, vista_discount_value, prazo_discount_value,
      ...
      // Verificar se a proposta não está expirada
      if (proposal.end_date && new Date(proposal.end_date) < new Date()) {
        return null;
      }
  ...
  async getStats() {
    try {
      const { data: proposals, error } = await supabase
        .from('proposals')
        .select('id, status, total_value, end_date, created_at');
  ...
  async findByPublicTokenComplete(token) {
    try {
      ...
      const { data: proposal, error: proposalError } = await supabase
        ...
        .select(`
          id, proposal_number, client_id, type, status, total_value, end_date,
          unique_link, link_expires_at, signed_at, max_installments,
  ...
  async findAcceptedProposals(filters = {}) {
    try {
      ...
      if (filters.start_date) {
        query = query.gte('signed_at', filters.start_date);
      }

      if (filters.end_date) {
        query = query.lte('signed_at', filters.end_date);
      }
