...
class ReportService {
    ...
    async generateCommercialReportPDF(startDate, endDate, clientId) {
        ...
        const conversionRate = totalSentCount > 0 ? (totalSignedCount / totalSentCount * 100).toFixed(1) : 0;
        ...
        pdfGenerator.addTable(doc, {
            headers: [
                { label: 'Cliente', x: 50, width: 120 },
                { label: 'Nº Proposta', x: 170, width: 90 },
                { label: 'Status', x: 260, width: 80 },
                { label: 'Data', x: 340, width: 70 },
                { label: 'Valor', x: 410, width: 135, align: 'right' }
            ],
            rows: proposals.map(p => [
                this.getClientNameFromProposal(p).toUpperCase(),
                p.proposal_number,
                this.translateProposalStatus(p.status),
